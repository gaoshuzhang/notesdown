...
packages sp. The [sp_gallery.Rmd](sp_gallery.Rmd) and
[sp_gallery.R](sp_gallery.R) files are available here, or on
[github](https://github.com/edzer/sp/tree/gh-pages). This html file
was generated from the R markdown file (sp_gallery.Rmd) by
...
bgMap = get_map(as.vector(bb), source = "google") # WGS84 for background map
plot(spTransform(meuse, merc), bgMap = bgMap, pch = 16, cex = .5)
```
...
`coord_map()` or `coord_quickmap()` to instruct `ggplot` this
concerns longitude/latitude coordinates (not stating the datum).
`coord_map()` projects to mercator (by default), `coord_quickmap()`
computes a suitable aspect ratio and plots degrees.
...
Bubble plots can be generated through `ggplot` by
...
The [ggplot2 wiki](https://github.com/hadley/ggplot2/wiki/plotting-polygon-shapefiles)
indicates that plotting polygons with holes may be problematic with ggplot.

## Other packages for making maps

The [spatial task
view](https://cran.r-project.org/web/views/Spatial.html) contains
pointers to other CRAN packages that have functionality for creating
general or custom maps, in addition to pointers to packages that
can be used for analyzing spatial data.
