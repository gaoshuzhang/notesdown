...
showtext 包[@Qiu2015]可以调用系统字体，图\@ref(fig:demo-ggplot2)使用5号思源宋体，英文和数字使用 serif 字体。详细的使用文档可以看 [showtext 的开发页面](https://github.com/yixuan/showtext)
...
```{r xkcd-embed,fig.cap = "xkcd 风的图", fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png', dev= if (knitr:::is_latex_output()) 'cairo_pdf' else 'png'}
library(extrafont)
library(xkcd)
