...
```{r demo-ggplot2, fig.cap="showtext包处理图里的中文"}
library(ggplot2)
...
```{r map-Fiji-earthquake, fig.cap="斐济地震带"}
library(maps)
library(mapdata)
...
这篇文章主要使用 ggplot2 包[@Wickham2016]绘图，为了好玩我们还使用 xkcd 字体，先下载和加载 xkcd 包[@R-xkcd]，它提供一个 gg 风格的图层`theme_xkcd()`，图 `r ifelse(knitr::is_html_output(),'\\@ref(fig:xkcd-demo)','\\@ref(fig:xkcd-embed)') ` 是一个简单使用 xkcd 字体的例子

```{r xkcd-embed,fig.cap = "xkcd 风的图", eval= knitr::is_latex_output(),dev='cairo_pdf'}
library(ggplot2)
library(extrafont)
library(xkcd)
...
```{r xkcd-demo,fig.cap = "xkcd 风的图", eval= knitr::is_html_output(), echo=FALSE}
knitr::include_graphics("figures/xkcd_embed.png")
```

## fontcm

安装 fontcm 包[@R-fontcm]处理数学公式，下载 fontcm 包^[<https://github.com/wch/fontcm>] 和相关字体，使用 fontcm 包，一个小 demo 如下

```{r fontcm-demo, dev='cairo_pdf',fig.width=8,fig.height=4,out.width="100%",fig.cap="fontcm 处理数学公式"}
library(fontcm)
p <- qplot(c(1, 5), c(1, 5)) +
...
tikz 源于 LaTeX 宏包，在 R 语言绘图的世界里，对基础图形有很好的支持，如图 `r ifelse(knitr::is_html_output(),'\\@ref(fig:tikz-demo-png)','\\@ref(fig:tikz-demo)') `
先测试一下
...
```{r tikz-demo, eval = knitr::is_latex_output(), fig.cap="线性模型",echo=FALSE}
knitr::include_graphics(path = "figures/linear.pdf")
```
```{r tikz-demo-png, eval = knitr::is_html_output(), fig.cap="线性模型",echo=FALSE}
knitr::include_graphics(path = "figures/linear.png")
```
...
虽然复杂图片也可直接用 tikz 制作但是，尺寸等细节不好调整（我还没找到合适的方法），比较合适的做法可能是，独立制作图，然后作为图片插入，以一个颇具复杂度的图片为例，如图 `r ifelse(knitr::is_html_output(),'\\@ref(fig:plot3d-tikz-png)','\\@ref(fig:plot3d-tikz)') ` 所示
...
```{r plot3d-tikz, eval = knitr::is_latex_output(), fig.cap="tikz 复杂公式",echo=FALSE}
knitr::include_graphics(path = "figures/binormal.pdf")
```
```{r plot3d-tikz-png, eval = knitr::is_html_output(), fig.cap="tikz 复杂公式",echo=FALSE}
knitr::include_graphics(path = "figures/binormal.png")
```
