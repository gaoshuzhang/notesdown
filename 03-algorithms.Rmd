...
为描述方便起见， 令 $\bm{\theta}^{\top} = (\sigma^2,\phi,\tau^2)$， 这里和之前先验分布一节的$\bm{\theta}$含义一样， 模型参数构成的一个集合， 只是没有将 $\beta$ 纳入进来， 贝叶斯框架下， 要求对所有未知参数给定先验分布， 包括模型系数。
...
&= E_{T|y}\left[\frac{N(t;D\beta,\Sigma(\theta))}{N(t;D\beta_{0},\Sigma(\theta_{0}))}\right]
\end{aligned}
\end{equation}
