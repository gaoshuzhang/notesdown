...
```{r log,fig.showtext=TRUE, dev = if(knitr::is_html_output()) 'svg' else if(knitr::is_latex_output()) 'cairo_pdf' else 'png'}
library(ggplot2)
library(cranlogs)
library(ggthemes)
...
基于 JS 动态库的 R 包 leaflet [@R-leaflet]， rbokeh [@R-rbokeh] 和 plotly [@R-plotly] ，我比较喜欢 highcharter [@R-highcharter]，美观高效，还活跃开发中。

```{r highcharter, eval= knitr::is_html_output(), fig.cap="highcharter 图形"}
library("highcharter")
...
```{r rbokeh, eval = knitr::is_html_output(), fig.cap="rbokeh 图形",warning=FALSE,message=FALSE}
library(rbokeh)
...
```{r plotly, eval = knitr::is_html_output(), fig.cap="plotly 图形"}
library(plotly)
