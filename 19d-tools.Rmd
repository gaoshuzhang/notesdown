...
```{bash}
tex_pkg <- system("tlmgr list --only-installed", intern = TRUE)
do.call(rbind, strsplit(tex_pkg, split = ":"))[, 1]
```

系统安装的字体

```{r}
fonts <- system("fc-list :lang=en | sort", intern = TRUE)
...
```bash
git init
git remote add origin https://github.com/XiangyunHuang/notes-book.git
git add -A
