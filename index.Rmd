...
图 \@ref(fig:xkcd-demo) 是一个简单使用 xkcd 字体的例子
...
```{r download-emojifont, eval = (!"emojifont" %in% .packages(TRUE))}
...
```
画一个玫瑰
```{r emoji-ggplot,dev='png'}
ggplot() + geom_emoji("rose", color='steelblue') + theme_void()
```
fontAwesome 字体使用

```{r emoji-ggplot-demo2,dev='png'}
set.seed(2018)
